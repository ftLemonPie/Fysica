...
\end{abstract}
\maketitle
\section{Oefeningen coulombkracht op 1 lijn}
%———————————————————————————————————————
%: OEFENINGEN 1 LIJN
%———————————————————————————————————————
...
\begin{exercise}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \node[above] at (0,\R) {$Q_1= +2{,}0  \, \mathrm{\mu C}$};
            \node[above, xshift=-0.8cm, yshift=0.1cm] at (5,\R) {$Q_2= +3{,}0  \, \mathrm{\mu C}$};
            \node[above] at (10,\R) {$Q_3= -4{,}0 \cdot  \, \mathrm{\mu C}$};
        \end{tikzpicture}
    \end{center}
        ...
        \begin{center}
            \begin{tikzpicture}
               \useasboundingbox (-1, -1.0) rectangle (11, 1.5);
                ...
                % FORCES
                \draw[kracht, transform canvas={yshift=+2.0pt}] (B) -- (5+2.1/2,0) node[above right] {$\vec{\mathbf{F}}_{12}$};
                \draw[kracht, transform canvas={yshift=-2.0pt}] (B) -- (5+4.3/2,0) node[below left] {$\vec{\mathbf{F}}_{32}$};
                \draw[resulterendekracht] (B) -- (5+6.4/2,0) node[right] {$\vec{\mathbf{F}}_{\text{res}}$};
...
% OEF LIJN2: EVENWICHT
\begin{exercise}
    Twee positieve puntladingen $Q_{1}= +4{,}0  \, \mathrm{\mu C}$ bij $x= 0{,}00 \, \mathrm{m}$ en $Q_{3}= +1{,}0  \, \mathrm{\mu C}$ bij $x= 0{,}80 \, \mathrm{m}$.  
    ...
    \begin{solution}\nl
        ...
        \begin{center}
            \begin{tikzpicture}
               \useasboundingbox (-1, -1.5) rectangle (9, 2.0);
                ...
                % FORCES on q (absolute coördinaten om Ximera te helpen)
                \draw[kracht, transform canvas={yshift=+2.0pt}] (B) -- (7.13,0) node[above right] {$\vec{\mathbf{F}}_{1q}$};
                \draw[kracht, transform canvas={yshift=-2.0pt}] (B) -- (3.53,0) node[above left] {$\vec{\mathbf{F}}_{3q}$};
        ...
        Krachten op \(q\):
        \[
            F_{1q}=k\cdot \frac{\lvert Q_{1}\rvert \cdot \lvert q \rvert}{(x-0 \, \mathrm{m})^{2}}\quad\text{(naar +x als \(q\) tussen de ladingen ligt)},
        \]
        \[
            F_{3q}=k\cdot \frac{\lvert Q_{3}\rvert \cdot \lvert q \rvert}{(0{,}80 \, \mathrm{m}-x)^{2}}\quad\text{(naar -x als \(q\) tussen de ladingen ligt)}.
        \]
        ...
        \[
            x=\frac{0{,}80 \, \mathrm{m}}{1{,}5}= 0{,}53333 \, \mathrm{m}\simeq 5,3\cdot 10^{-1} \, \mathrm{m}.
        \]

        Conclusie: de positieve testlading staat in evenwicht op \(x \approx 5,3\cdot 10^{-1} \, \mathrm{m}\), gerekend vanaf \(Q_{1}\).
    \end{solution}
\end{exercise}
%———————————————————————————————————————
...
\begin{exercise}
    Drie ladingen op één lijn: $Q_{1}= +8{,}0  \, \mathrm{nC}$ bij $x=0{,}00 \, \mathrm{m}$, $Q_{2}= +2{,}0  \, \mathrm{nC}$ bij $x=20 \, \mathrm{cm}$, en $Q_{3}$ bij $x=60 \, \mathrm{cm}$.  
            ...
            \begin{center}
                \begin{tikzpicture}
                   \useasboundingbox (-1, -1.5) rectangle (7, 2.0);
                    ...
                    % FORCES on Q2 (absolute coördinaten)
                    \draw[kracht, transform canvas={yshift=+2.0pt}] (B) -- (3.5,0) node[above right] {$\vec{\mathbf{F}}_{12}$};
                    \draw[kracht, transform canvas={yshift=-2.0pt}] (B) -- (0.5,0) node[below left] {$\vec{\mathbf{F}}_{32}$};
    ...
    \begin{question}
        ...
        \[ \lvert Q_{3} \rvert = \answer[tolerance=0.5,onlineshowanswerbutton]{3.2} \cdot 10^{-8} \, \mathrm{C} \]
        ...
        \begin{solution}\nl
            \underline{Gegevens:}
            \begin{align*}
                & Q_{1} = +8{,}0  \, \mathrm{nC}= +8{,}0 \cdot 10^{-9} \, \mathrm{C} & & x_{1} = 0{,}00 \, \mathrm{m}\\
                & Q_{2} = +2{,}0  \, \mathrm{nC}= +2{,}0 \cdot 10^{-9} \, \mathrm{C} & & x_{2} = 0{,}20 \, \mathrm{m}\\
            ...
            \[
                ...
                = (8{,}0 \cdot 10^{-9} \, \mathrm{C}) \cdot \frac{(0{,}40 \, \mathrm{m})^{2}}{(0{,}20 \, \mathrm{m})^{2}}.
            \]
            Berekening: \((0{,}40 \, \mathrm{m})^{2}=0{,}16 \, \mathrm{m^2}\) en \((0{,}20 \, \mathrm{m})^{2}=0{,}04 \, \mathrm{m^2}\), dus de verhouding is \(=0{,}16/0{,}04=4\).
            \[
                \lvert Q_{3} \rvert = (8{,}0 \cdot 10^{-9} \, \mathrm{C}) \cdot 4 = 3{,}2 \cdot 10^{-8} \, \mathrm{C}.
            \]
    
            Conclusie: Gezien we eerder hebben beredeneerd dat het teken positief is, is de lading $Q_{3} = +3{,}2 \cdot 10^{-8} \, \mathrm{C}$ om \(Q_{2}\) in evenwicht te houden.
        \end{solution}
    \end{question}
\end{exercise}
%———————————————————————————————————————
            ...
            \begin{center}
                \begin{tikzpicture}
                   \useasboundingbox (-1, -1.0) rectangle (9, 1.5);
                    ...
                    % FORCES on Q3 (absolute coördinaten)
                    \draw[kracht, transform canvas={yshift=+2.0pt}] (C) -- (4.5,0) node[above left] {$\vec{\mathbf{F}}_{23}$};
                    \draw[kracht, transform canvas={yshift=-2.0pt}] (C) -- (8.0,0) node[below right] {$\vec{\mathbf{F}}_{13}$};
                    \draw[resulterendekracht] (C) -- (5.5,0) node[below] {$\vec{\mathbf{F}}_{\text{res}}$};
...
\begin{exercise}
    ...
    \begin{question}
        ...
        \begin{solution}\nl
            ...
            \begin{center}
                \begin{tikzpicture}
                   \useasboundingbox (-5.5, -1.5) rectangle (7.5, 2.0);
                    ...
                    % FORCES on q (absolute coördinaten om Ximera te helpen, testlading op -3)
                    \draw[kracht, transform canvas={yshift=+2.0pt}] (Q) -- (-1.5,0) node[above right] {$\vec{\mathbf{F}}_{1q}$};
                    \draw[kracht, transform canvas={yshift=-2.0pt}] (Q) -- (-4.5,0) node[below left] {$\vec{\mathbf{F}}_{2q}$};
            ...
            \[
                k\cdot \frac{\lvert Q_{1} \rvert \cdot  \lvert q\rvert}{d^{2}} = k\cdot \frac{\lvert Q_{2} \rvert \cdot \lvert q\rvert}{(0{,}60 \, \mathrm{m} + d)^{2}}
            \]
            ...
            \]
            \[
                d = 0{,}60 \, \mathrm{m}= 6,0\cdot 10^{-1} \, \mathrm{m}
            \]
...
\begin{exercise}
    Drie puntladingen liggen op één lijn. Lading $Q_1 = -50  \, \mathrm{mC}$ ligt uiterst links. Op een afstand van $0{,}400 \, \mathrm{m}$ rechts van $Q_1$ bevindt zich lading $Q_3 = -20 \, \mathrm{mC}$. Nog eens $0{,}200 \, \mathrm{m}$ verder naar rechts ligt lading $Q_2 = +100 \cdot 10^{-6} \, \mathrm{C}$. 
    ...
    \begin{solution}\nl
        \underline{Gegevens:}
        \begin{align*}
            & Q_{1} = -50 \cdot 10^{-3} \, \mathrm{C} & & r_{13} = 0{,}400 \, \mathrm{m}\\
            & Q_{2} = +100 \cdot 10^{-6} \, \mathrm{C} & & r_{32} = 0{,}200 \, \mathrm{m}\\
            & Q_{3} = -20 \cdot 10^{-3} \, \mathrm{C} & & k = 8{,}99 \cdot 10^9 \, \mathrm{\frac{N \cdot m^2}{C^2}}
        \end{align*}
        ...
        \begin{align*}
            ...
            &= \frac{8{,}99 \cdot 10^9 \, \mathrm{\frac{N \cdot m^2}{C^2}} \cdot 50 \cdot 10^{-3} \, \mathrm{C} \cdot 20 \cdot 10^{-3} \, \mathrm{C}}{(0{,}400 \, \mathrm{m})^2} \\
            &= 56187500 \, \mathrm{N} \simeq 5{,}6\cdot 10^{7} \, \mathrm{N}
        \end{align*}
        ...
        \begin{align*}
            ...
            &= \frac{8{,}99 \cdot 10^9 \, \mathrm{\frac{N \cdot m^2}{C^2}} \cdot 100 \cdot 10^{-6} \, \mathrm{C} \cdot 20 \cdot 10^{-3} \, \mathrm{C}}{(0{,}200 \, \mathrm{m})^2} \\
            &= 449500 \, \mathrm{N}\simeq 4{,}5\cdot 10^{5} \, \mathrm{N}
        \end{align*}
        ...
        \[
            F_{\text{res}} = F_{13} + F_{23} = 56187500 \, \mathrm{N} + 449500 \, \mathrm{N} = 56637000 \, \mathrm{N} \simeq 5,7\cdot 10^{7} \, \mathrm{N}
        \]
