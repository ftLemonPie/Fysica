...
\begin{question}
    Een positieve testlading van $+3\mu\mathrm{C}$ wordt geplaatst in punt \textit{P} in de ruimte. Deze ondervindt een extern elektrisch veld dat naar rechts is gericht met een grootte van $4 \cdot 10^6\mathrm{ N/C}$. Stel dat we deze testlading weghalen, en vervangen door een nieuwe testlading van $-3\mu\mathrm{C}$. Wat gebeurt er dan met het externe elektrisch veld in punt \textit{P}?
...
% \begin{example}\label{vb:elektrisch_veld_2ladingen}
%     Ladingen $q_1$ en $q_2$ liggen op de x-as, respectievelijk op een afstand $a$ en $b$ van het nulpunt; zoals te zien op figuur \ref{fig:elektrisch_veld_2ladingen}.
%     \begin{center}
%         \captionsetup{width=0.85\textwidth}
%         \includegraphics[width=0.6\textwidth]{pic5/elektriciteit/veld/vb_elektrisch_veld_driehoek.png}
%         % TIP: Gebruik Figure P23.12 uit de bijlage.
%         \captionof{figure}{(Oefening \ref{vb:elektrisch_veld_2ladingen}) Het totaal elektrisch veld $\vec{E}$ in punt \textit{P} is de vector som $\vec{E_1}+\vec{E_2}
%         $, waar $\vec{E_1}$ het elektrisch veld is van de positieve lading $q_1$ en $\vec{E_2}$ het veld is door de negatieve lading $q_2$ }
%         \label{fig:elektrisch_veld_2ladingen}
%     \end{center}
%     \begin{question}
%     Vind the componenten
%     \end{question}
%     \begin{solution}
